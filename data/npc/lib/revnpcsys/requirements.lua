...
if not NpcRequirements then
    ...
    function NpcRequirements:checkStorageValue(player)
        ...
        if storage.operator == "==" then
            return storageValue == storage.value, MESSAGE_LIST.storage:replaceTags({storage = {key = storage.key, value = storage.value}})
        elseif storage.operator == "~=" then
            return storageValue ~= storage.value, MESSAGE_LIST.storageNotEqual:replaceTags({storage = {key = storage.key, value = storage.value}})
        elseif storage.operator == "<" then
            if storage.value2 and storage.operator2 == ">" then
                return storageValue < storage.value and storageValue > storage.value2, MESSAGE_LIST.storageRange:replaceTags({storage = {key = storage.key, value = storage.value, value2 = storage.value2}})
            elseif storage.value2 and storage.operator2 == ">=" then
                return storageValue < storage.value and storageValue >= storage.value2, MESSAGE_LIST.storageRange:replaceTags({storage = {key = storage.key, value = storage.value, value2 = storage.value2}})
            end
            return storageValue < storage.value, MESSAGE_LIST.storageToLow:replaceTags({storage = {key = storage.key, value = storage.value}})
        elseif storage.operator == ">" then
            if storage.value2 and storage.operator2 == "<" then
                return storageValue > storage.value and storageValue < storage.value2, MESSAGE_LIST.storageRange:replaceTags({storage = {key = storage.key, value = storage.value, value2 = storage.value2}})
            elseif storage.value2 and storage.operator2 == "<=" then
            ...
            return storageValue > storage.value, MESSAGE_LIST.storageToHigh:replaceTags({storage = {key = storage.key, value = storage.value}})
        elseif storage.operator == "<=" then
            if storage.value2 and storage.operator2 == ">" then
                return storageValue <= storage.value and storageValue > storage.value2, MESSAGE_LIST.storageRange:replaceTags({storage = {key = storage.key, value = storage.value, value2 = storage.value2}})
            elseif storage.value2 and storage.operator2 == ">=" then
